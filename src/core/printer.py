...
def _monitor_device_queue(printer_name: str) -> None:
    """
        ...
        while True:
            ...
            if jobs:
                ...
                if status & win32print.JOB_STATUS_ERROR:
                    raise RuntimeError("La impresora reportó un error crítico de hardware.")
                elif status & win32print.JOB_STATUS_PAPEROUT:
                    raise RuntimeError("La impresora se ha quedado sin papel o está atascada.")
                elif status & win32print.JOB_STATUS_OFFLINE:
                    raise RuntimeError("La impresora cambió a estado fuera de línea.")
...
def print_image_directly(file_path: str, printer_name: str, page_type: str = "letter") -> None:
    ...
    dib.draw(hdc.GetSafeHdc(), (x_offset, y_offset, x_offset + draw_w, y_offset + draw_h))
...
def send_to_system_printer(file_path: str, printer_name: str = None, page_type: str = "letter") -> None:
    ...
    if sys.platform != "win32":
        raise RuntimeError("Direct hardware printing tracking is only optimized for Windows")
    ...
    if file_path.lower().endswith('.pdf'):
        ...
        print_image_directly(file_path=file_path, printer_name= printer_name, page_type=page_type)
